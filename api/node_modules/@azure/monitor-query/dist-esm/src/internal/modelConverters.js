...
export function convertRequestForMetrics(metricNames, queryMetricsOptions) {
    ...
    const { orderBy, aggregations, metricNamespace, timespan, granularity, rollUpBy } = queryMetricsOptions, rest = __rest(queryMetricsOptions, ["orderBy", "aggregations", "metricNamespace", "timespan", "granularity", "rollUpBy"]);
...
export function convertResponseForMetrics(generatedResponse) {
    ...
    const { resourceregion, value: _ignoredValue, interval, timespan } = generatedResponse, rest = __rest(generatedResponse, ["resourceregion", "value", "interval", "timespan"]);
...
export function convertRequestOptionsForMetricsDefinitions(options) {
    ...
    const { metricNamespace } = options, rest = __rest(options, ["metricNamespace"]);
    const obj = Object.assign({}, rest);
    if (metricNamespace) {
        obj.metricnamespace = metricNamespace;
    }
...
export function convertResponseForMetricsDefinitions(generatedResponse) {
    const definitions = generatedResponse === null || generatedResponse === void 0 ? void 0 : generatedResponse.map((genDef) => {
        const { name, dimensions, displayDescription, metricAvailabilities } = genDef, rest = __rest(genDef, ["name", "dimensions", "displayDescription", "metricAvailabilities"]);
...
export function convertResponseForMetricNamespaces(generatedResponse) {
    const namespaces = generatedResponse === null || generatedResponse === void 0 ? void 0 : generatedResponse.map((genDef) => {
        const { properties } = genDef, rest = __rest(genDef, ["properties"]);
        const response = Object.assign({}, rest);
        if (properties) {
...
export function computeResultType(generatedResponse) {
    ...
    if (!generatedResponse.error) {
        ...
        if (generatedResponse.tables) {
            ...
            const result = Object.assign({ status: LogsQueryResultStatus.Failure }, errorInfo);
            return result;
        }
    }
}
